return {
    ...
    autoHideUI                        = {
        value = g_platform.isMobile() and true or false,
        action = function(value, options, controller, panels, extraWidgets)
            if modules.game_autohide then
                modules.game_autohide.setAutoHideEnabled(value)
            end
        end
    },
    uiScale                           = {
        value = 100,
        action = function(value, options, controller, panels, extraWidgets)
            -- Scale is stored as percentage (75-125)
            panels.interfaceHUD:recursiveGetChildById('uiScale'):setText(string.format('UI Scale: %d%%', value))
        end
    },
    ...
    joystickFloating                  = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setFloatingMode then
                modules.game_joystick.setFloatingMode(value)
            end
        end
    },
    joystickLeftHanded                = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('leftHanded', value)
            end
        end
    },
    hapticEnabled                     = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('hapticEnabled', value)
            end
        end
    },
    joystickAutoHide                  = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('autoHide', value)
            end
        end
    },
    joystickOpacity                   = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('opacity', value / 100)
            end
            if panels.mobilePanel then
                local widget = panels.mobilePanel:recursiveGetChildById('joystickOpacity')
                if widget then
                    widget:setText(string.format('Joystick Opacity: %d%%', value))
                end
            end
        end
    },
    joystickSize                      = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('size', value)
            end
            if panels.mobilePanel then
                local widget = panels.mobilePanel:recursiveGetChildById('joystickSize')
                if widget then
                    widget:setText(string.format('Joystick Size: %dpx', value))
                end
            end
        end
    },
    joystickDeadZone                  = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('deadZone', value / 100)
            end
            if panels.mobilePanel then
                local widget = panels.mobilePanel:recursiveGetChildById('joystickDeadZone')
                if widget then
                    widget:setText(string.format('Dead Zone: %d%%', value))
                end
            end
        end
    },
    joystickSensitivity               = {
        ...
        action = function(value, options, controller, panels, extraWidgets)
            if not g_platform.isMobile() then return end
            if modules.game_joystick and modules.game_joystick.setSetting then
                modules.game_joystick.setSetting('sensitivity', value / 100)
            end
            if panels.mobilePanel then
                local widget = panels.mobilePanel:recursiveGetChildById('joystickSensitivity')
                if widget then
                    widget:setText(string.format('Sensitivity: %d%%', value))
                end
            end
        end
    }
