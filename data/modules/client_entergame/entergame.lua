...
function EnterGame.init()
    ...
    if Servers_init then
        -- Populate server selector if multiple servers
        if table.size(Servers_init) > 1 then
            EnterGame.populateServerSelector()
        end

        if table.size(Servers_init) == 1 then
            local serverName, valuesInit = next(Servers_init)
            local serverHost = valuesInit.host or serverName
            EnterGame.setUniqueServer(serverHost, valuesInit.port, valuesInit.protocol)
            EnterGame.setHttpLogin(valuesInit.httpLogin)
        elseif not host or host == "" then
            local serverName, valuesInit = next(Servers_init)
            local serverHost = valuesInit.host or serverName
            EnterGame.setDefaultServer(serverHost, valuesInit.port, valuesInit.protocol)
            EnterGame.setHttpLogin(valuesInit.httpLogin)
        end
    else
...
function EnterGame.populateServerSelector()
    local serverSelector = enterGame:getChildById('serverSelector')
    if not serverSelector then
        return
    end
    ...
    local sortedServers = {}
    for serverName, _ in pairs(Servers_init) do
        table.insert(sortedServers, serverName)
    end
    table.sort(sortedServers)

    for _, serverName in ipairs(sortedServers) do
        serverSelector:addOption(serverName, serverName)
    end

    -- Connect the change event
    connect(serverSelector, {
        onOptionChange = EnterGame.onServerSelectorChange
    })

    -- Select the first server by default
    if #sortedServers > 0 then
        serverSelector:setCurrentOption(sortedServers[1])
    end
end

function EnterGame.onServerSelectorChange(comboBox, option)
    if not Servers_init or not option then
        return
    end

    local serverData = Servers_init[option]
    ...
    -- Use the host field if available, otherwise use the server name as host
    local serverHost = serverData.host or option
    hostTextEdit:setText(serverHost)
    portTextEdit:setText(tostring(serverData.port))

    -- Set protocol version
    if serverData.protocol and clientBox then
        clientBox:setCurrentOption(serverData.protocol)
    end

    -- Set HTTP login checkbox
    if httpLoginBox then
        httpLoginBox:setChecked(serverData.httpLogin or false)
    end
end
